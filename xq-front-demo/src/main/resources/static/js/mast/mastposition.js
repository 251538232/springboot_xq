$(function () {
    $("#gridTable").jqGrid({
        url: apiUrl + 'mastPosition/list?access_token=' + getToken("access_token"),
        datatype: "json",
        colModel: [
			{ label: '役職', name: 'id', width: 80 }, 
			{ label: '職務コード', name: 'code', width: 80 }, 
			{ label: '職務名', name: 'name', width: 80 }, 
			{ label: '職務レベル', name: 'level', width: 80 }, 
...
var vm = new Vue({
	...
	methods: {
        ...
        createOrUpdate: function (event) {
			var url = vm.mastPosition.id == null ? "mastPosition/create" : "mastPosition/update";
			$.ajax({
				type: "POST",
			    url: apiUrl + url,
			    contentType: "application/json",
			    data: JSON.stringify(vm.mastPosition),
			    success: function(r) {
			    	if(r.code === 0) {
						var msg = vm.mastPosition.id == null ? "データ新規成功しました" : "データ更新成功しました";
		...
		del: function (event) {
			...
			confirm('選択したデータを削除しますか？', function() {
				$.ajax({
					type: "GET",
				    url: apiUrl + "mastPosition/delete/" + ids.join(","),
				    success: function(r) {
						if(r.code == 0) {
		...
		getDetail: function(id) {
            $.ajax({
				type: "GET",
				url: apiUrl + "mastPosition/detail/"+id,
                success: function(r) {
					if(r.code == 0) {
                        vm.mastPosition = r.data;
					} else {
						alert(r.message);
					}
				}
